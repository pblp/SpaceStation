...
*/
		...
		space_drugs
			...
			on_mob_life(var/mob/living/M as mob)
				...
				if(prob(7)) M:emote(pick("дергаетс&#255;","пускает слюнки","постанывает","хихикает"))
				holder.remove_reagent(src.id, 0.2)
				return
			...
			on_mob_life(var/mob/living/M as mob)
				...
				switch(data)
					if(1 to 5)
						if (!M:stuttering) M:stuttering = 1
						M.make_dizzy(5)
						if(prob(10)) M:emote(pick("осматриваетс&#255;","нервно хихикает"))
					if(5 to 10)
						if (!M:stuttering) M:stuttering = 1
						M.make_jittery(10)
						M.make_dizzy(10)
						M.druggy = max(M.druggy, 35)
						if(prob(20)) M:emote(pick("осмариваетс&#255;","нервно хихикает"))
					if (10 to INFINITY)
						if (!M:stuttering) M:stuttering = 1
						M.make_jittery(20)
						M.make_dizzy(20)
						M.druggy = max(M.druggy, 40)
						if(prob(30)) M:emote(pick("осматриваетс&#255;","нервно хихикает"))
				holder.remove_reagent(src.id, 0.2)
				data++
				..()
				return
