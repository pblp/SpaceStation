	...
	switch(M.a_intent)
		...
		if ("hurt")
			...
			if (prob(90))
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("\red <B>[] ударил []!</B>", M, src), 1)
				if (damage > 9||prob(5))//Regular humans have a very small chance of weakening an alien.
					Weaken(1,5)
					for(var/mob/O in viewers(M, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("\red <B>¬незапно —»Ћ№Ќџ… удар!</B>"), 1, "\red ќй.", 2)
				adjustBruteLoss(damage)
				updatehealth()
			else
				...
				for(var/mob/O in viewers(src, null))
					if ((O.client && !( O.blinded )))
						O.show_message(text("\red <B>[] промазал, пыта&#255;сь ударить []!</B>", M, src), 1)
		...
		if ("disarm")
			if (!lying)
				if (prob(5))//Very small chance to push an alien down.
					...
					for(var/mob/O in viewers(src, null))
						if ((O.client && !( O.blinded )))
							O.show_message(text("\red <B>[] толкнул [] на пол!</B>", M, src), 1)
				else
					if (prob(50))
						drop_item()
						playsound(loc, 'thudswoosh.ogg', 50, 1, -1)
