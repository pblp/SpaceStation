...
/mob/living/carbon/human/proc/attacked_by(var/obj/item/I, var/mob/living/carbon/human/user, var/def_zone)
	...
	visible_message("\red <B>[user] ударил [src] в [hit_area] при помощи [I.name]!</B>")
//	flash_weak_pain()

	if((user != src) && check_shields(I.force, "[I.name]"))
		return 0
	var/armor = run_armor_check(affecting, "melee", "Одежда защитила от удара в [hit_area].", "Одежда забрала себе часть удара в [hit_area].")
	if(armor >= 2)	return 0
	if(!I.force)	return 0
		...
		switch(hit_area)
			if("head")//Harder to score a stun but if you do it lasts a bit longer
				if(prob(I.force))
					...
					visible_message("\red <B>[src] потер&#255;л сознание!</B>")
//					flash_weak_pain()
					if(src != user)
			...
			if("chest")//Easier to score a stun but lasts less time
				if(prob((I.force + 10)))
					...
					visible_message("\red <B>[src] полетел вниз, на пол!</B>")
//					flash_weak_pain()
