/mob/living/carbon/human/attack_hand(mob/living/carbon/human/M as mob)
	...
	switch(M.a_intent)
		...
		if("hurt")
			...
			visible_message("\red <B>[M] ударил [src] в [M.zone_sel.selecting]!</B>")
			...
			if(damage >= 9)
				visible_message("\red <B>Внезапно СИЛЬНЫЙ удар!!</B>")
		...
		if("disarm")
			...
			if (randn <= 25)
				...
				playsound(loc, 'thudswoosh.ogg', 50, 1, -1)
				visible_message("\red <B>[M] толкнул [src]!</B>")
				return
			...
			playsound(loc, 'punchmiss.ogg', 25, 1, -1)
			visible_message("\red <B>[M] попыталс&#255; толкнуть [src]!</B>")
	return
