...
/mob/living/carbon/human/proc/smokeneed()
	if(cigarettes_need==0)
		smokewant+=1
		sleep(30)
		if(smokewant==100)
			src << "<B>\red *Перекур...*</B>"
		if(smokewant==200)
			src << "<B>\red *Нужно срочно выкурить сигарету...*</B>"
		if(smokewant==300)
			src << "<B>\red *МНЕ НУЖЕН НИКОТИН!*</B>"
			sleep(500)
			src << "<B>\red *Сигареты!*</B>"
		return
			...
			spawn(48)
				if(addiction == "Алкоголь")
					if(!reagents.has_reagent("vodka", 10) || !reagents.has_reagent("tequilla", 10) || !reagents.has_reagent("vermouth", 10) || !reagents.has_reagent("wine", 10) || !reagents.has_reagent("kahlua", 10) || !reagents.has_reagent("cognac", 10) || !reagents.has_reagent("ale", 10) || !reagents.has_reagent("whiskey", 10))
						if(prob(5))
							if(prob(3))
								src << "<B>\red *Мне нужен алкоголь....*</B>"
				if(addiction == "Никотин")
					smokeneed()
