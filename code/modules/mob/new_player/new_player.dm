...
/mob/new_player
	...
	proc/new_player_panel_proc()
		var/output = "<B>Игра</B>"
		output +="<hr>"
		output += "<br><a href='byond://?src=\ref[src];show_preferences=1'>Настройка персонажа</A><BR><BR>"

		if(!ticker || ticker.current_state <= GAME_STATE_PREGAME)
			if(!ready)	output += "<a href='byond://?src=\ref[src];ready=1'>Готовность</A><BR>"
			else	output += "<b>Ты готов</b> (<a href='byond://?src=\ref[src];ready=2'>Отменить</A>)<BR>"

		else
			//output += "<a href='byond://?src=\ref[src];manifest=1'>View the Crew Manifest</A><BR><BR>"
			output += "<a href='byond://?src=\ref[src];late_join=1'>Зайти!</A><BR>"
