...
*/
/*
/obj/item/weapon/training/attack(var/mob/src, var/obj/item/I, var/mob/living/carbon/human/user)
	var/target_area = user.zone_sel.selecting
	var/parry_area = src.zone_sel.selecting
	for(var/mob/O in viewers(src,7))
		O << "\red \b [user.name] атакует [src] в [target_area] при помощи [I.name]!"
	if(!parry_area == target_area) //parrying occurs here
		if(I = /obj/item/weapon/training)
			if(prob(chance_parry))
				for(var/mob/O in viewers(src,7))
					O << "\red \b [src] парирует атаку своим [I.name]!"
					return
//	src.adjustBruteLoss(-src.force)
