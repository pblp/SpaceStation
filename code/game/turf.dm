/turf/DblClick()
	...
	if(usr.hand && istype(usr.l_hand, /obj/item/weapon/flamethrowinger))
		var/turflist = getline(usr,src)
		var/obj/item/weapon/flamethrowinger/F = usr.l_hand
		F.flame_turf(turflist)
	else if(!usr.hand && istype(usr.r_hand, /obj/item/weapon/flamethrowinger))
		var/turflist = getline(usr,src)
		var/obj/item/weapon/flamethrowinger/F = usr.r_hand
		F.flame_turf(turflist)
			...
			if(istype(H.shoes, /obj/item/clothing/shoes))
				if(H.m_intent == "run")
					if(H.footstep >= 2)
						...
						H.footstep++
					if(H.footstep == 0)
						playsound(src, "step", 50, 1) // this will get annoying very fast.
				else
					playsound(src, "step", 20, 1)
	...
	if(is_grass_floor())
		if(!broken && !burnt)
			if(!(icon_state in list("grass1","grass2","grass3","grass4")))
				icon_state = "grass[pick("1","2","3","4")]"
	spawn(1)
		if(istype(src,/turf/simulated/floor)) //Was throwinging runtime errors due to a chance of it changing to space halfway through.
			if(air)
				update_visuals(air)
