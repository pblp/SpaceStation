	...
	switch(spideros)
		...
		if(4)
			dat += {"
					...
					<li>*<b>Energy Blade</b> (<i>500E</i>) is a highly effective weapon. It is summoned directly to the ninja's hand and can also function as an EMAG for certain objects (doors/lockers/etc). You may also use it to cut through walls and disabled doors. Experiment! The blade will crit humans in two hits. This item cannot be placed in containers and when dropped or throwingn disappears. Having an energy blade drains more power from the battery each tick.</li>
...
/obj/effect/energy_net
	...
	process(var/mob/living/carbon/M as mob)
		...
		if(!isnull(src))//As long as both net and person exist.
			...
			M.loc = pick(holdingfacility)//throwing mob in to the holding facility.
