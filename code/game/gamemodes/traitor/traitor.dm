...
/datum/game_mode/proc/greet_traitor(var/datum/mind/traitor)
	traitor.current << sound('antagonist.ogg')
	traitor.current << "<B><font size=3 color=red>Ты - предатель!</font></B>"
	sleep(80)
	traitor.current << "<B>Запрещенна&#255; организаци&#255; 'Синдикат' стирает своим агентам пам&#255;ть о прошлой жизни, чтобы те могли сосредоточитьс&#255; на выполнении миссий. Все, что ты помнишь о себе - бесконечные физические и ментальные тренировки, которые, как кажетс&#255;, длились не меньше целой вечности. Ты живешь ради выполнени&#255; целей, остальное не имеет ни малейшего смысла.</B>"
	var/obj_count = 1
	for(var/datum/objective/objective in traitor.objectives)
		traitor.current << "<B>Задача #[obj_count]</B>: [objective.explanation_text]"
		obj_count++
	return
		...
		if(traitor.objectives.len)//If the traitor had no objectives, don't need to process this.
			...
			for(var/datum/objective/objective in traitor.objectives)
				if(objective.check_completion())
					world << "<B>Задача #[count]</B>: [objective.explanation_text] \green <B>УСПЕХ</B>"
				else
					world << "<B>Задача #[count]</B>: [objective.explanation_text] \red ПРОВАЛ?"
					traitorwin = 0
				count++

			if(traitorwin)
				world << "<B>Предатель выполнил свою миссию и разрушил последние надежды Нано-Трейзен на лидирование в гонке корпораций. Слава Синдикату!<B>"
			else
				world << "<B>Предатель провалил свою миссию?!<B>"
	return 1
