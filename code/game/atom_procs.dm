...
// TODO DOOHL: Intergrate params to new proc. Saved for another time because var/valid_place is a fucking brainfuck
	...
	if(ishuman(usr))
		...
		if(human.in_throwing_mode)
			return human.throwing_item(src)
		...
		if(monkey.in_throwing_mode)
			return monkey.throwing_item(src)
...
//		world << "atom.DblClick() on [src] by [usr] : src.type is [src.type]"
	...
	// ------- throwing -------
	if(usr.in_throwing_mode)
		return usr:throwing_item(src)
