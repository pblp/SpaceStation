...
/obj/machinery/bot/medbot
	name = "Medical Robot"
	desc = "Лучше сдохнуть чем так лечитьс&#255;!"
	icon = 'aibots.dmi'
	icon_state = "medibot0"
	layer = 5.0
	density = 1
...
/obj/machinery/bot/medbot/attack_hand(mob/user as mob)
	...
	if (.)
		return
	var/dat
	dat += "<TT><B>Автоматический железный врач</B></TT><BR><BR>"
	dat += "Статус: <A href='?src=\ref[src];power=1'>[src.on ? "On" : "Off"]</A><BR>"
	dat += "Пробирочка: "
	if (src.reagent_glass)
		dat += "<A href='?src=\ref[src];eject=1'>Загружено \[[src.reagent_glass.reagents.total_volume]/[src.reagent_glass.reagents.maximum_volume]\]</a>"
	else
		dat += "Не загружена"
	dat += "<br>Настройки поведени&#255; - [src.locked ? "locked" : "unlocked"]<hr>"
	if(!src.locked)
...
/obj/machinery/bot/medbot/process()
	...
	if(!src.patient)
		if(prob(1) && (!src.shut_up))
			var/message = pick("Я вас вылечу.","В-в-вылечу.","ВЫЛЕЧУ.","ВыЫЫылечУУУу.","Спасу!")
			src.speak(message)
			playsound(src.loc, 'medbot_target.ogg', 50, 1)
			...
			if(src.assess_patient(C))
				...
				spawn(0)
					if((src.last_newpatient_speak + 100) < world.time) //Don't spam these messages!
						var/message = pick("Эй, стой!","Подожди!","ОСТАНОВИСЬ!")
						src.speak(message)
						src.last_newpatient_speak = world.time
					src.visible_message("\blue <b>[src]</b> тычет своей штучкой в [C.name]!")
					playsound(src.loc, 'medbot_target.ogg', 50, 1)
				break
			else
				continue
...
/obj/machinery/bot/medbot/proc/medicate_patient(mob/living/carbon/C as mob)
	...
	if(C.stat == 2)
		var/death_message = pick("Ты что, заснул?","Вставай! Хах!","Я сделал все, что мог.")
	...
	if(!reagent_id) //If they don't need any of that they're probably cured!
		...
		src.currently_healing = 0
		src.last_found = world.time
		var/message = pick("Как новенький!","Сладенько.","Стань лучше!")
		src.speak(message)
		return
	else
		src.icon_state = "medibots"
		for(var/mob/O in viewers(src, null))
			O.show_message("\blue <B>[src] засовывает иголочку в [src.patient]!</B>", 1)
			var/message = pick("Подожди, делаю укол!","Стой смирно, а то промахнусь!","Сейчас будет больно!")
			src.speak(message)
		spawn(30)
			if ((get_dist(src, src.patient) <= 1) && (src.on))
				...
				for(var/mob/O in viewers(src, null))
					O.show_message("\blue <B>[src] делает особый укольчик [src.patient]!</B>", 1)
...
/obj/machinery/bot/medbot/proc/speak(var/message)
	if((!src.on) || (!message))
		return
	for(var/mob/O in hearers(src, null))
		O.show_message("<span class='game say'><span class='name'>[src]</span> бибикает, \"[message]\"",2)
		playsound(src.loc, 'medbot_target.ogg', 50, 1)
	return
...
/obj/machinery/bot/medbot/explode()
	src.on = 0
	for(var/mob/O in hearers(src, null))
		O.show_message("\red <B>[src] взрываетс&#255;!</B>", 1)
		new /obj/effect/decal/cleanable/robot_debris(src.loc,1)
		playsound(src.loc, 'medbot_pain.ogg', 50, 1)
