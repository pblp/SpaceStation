...
/obj/machinery/vending/proc/throwing_item()
	var/obj/throwing_item = null
	...
	for(var/datum/data/vending_product/R in src.product_records)
		...
		R.amount--
		throwing_item = new dump_path(src.loc)
		break
	if (!throwing_item)
		return 0
	spawn(0)
		throwing_item.throwing_at(target, 16, 3)
	src.visible_message("\red <b>[src] launches [throwing_item.name] at [target.name]!</b>")
	return 1
