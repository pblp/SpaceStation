...
*/
	...
	afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, flag)//TODO: go over this
		...
		if(last_throwing+4 > world.time)	return
		if(!host)
			del(src)
			return
		...
		if((get_dist(target, user) <= 16) && (get_dist(focus, user) <= 10))
			apply_focus_overlay()
			focus.throwing_at(target, 10, 1)
			last_throwing = world.time
		return


	proc/focus_object(var/obj/target)
		if(!istype(target,/obj))	return//Cant throwing non objects atm might let it do mobs later
		if(target.anchored)	return//No throwinging anchored things
		focus = target
		update_icon()
		apply_focus_overlay()
		return
