/obj/item/weapon/flamethrowinger/full/New(var/loc)
	..()
...
/obj/item/weapon/flamethrowinger
	name = "flamethrower"
	...
	update_icon()
		...
		if(lit)
			src.overlays += "+lit"
			item_state = "flamethrowinger_1"
		else
			item_state = "flamethrowinger_0"
		return
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/weapon/tank/plasma))
			if(src.ptank)
				user << "\red There appears to already be a plasma tank loaded in the flamethrowinger!"
				return
			src.ptank = W
			W.loc = src
			if (user.client)
	...
	attack_self(mob/user as mob)
		...
		if (!src.ptank)
			...
			return
		var/dat = text("<TT><B>Flamethrowinger (<A HREF='?src=\ref[src];light=1'>[lit ? "<font color='red'>Lit</font>" : "Unlit"]</a>)</B><BR>\n Tank Pressure: [src.ptank.air_contents.return_pressure()]<BR>\nAmount to throwing: <A HREF='?src=\ref[src];amount=-100'>-</A> <A HREF='?src=\ref[src];amount=-10'>-</A> <A HREF='?src=\ref[src];amount=-1'>-</A> [src.throwing_amount] <A HREF='?src=\ref[src];amount=1'>+</A> <A HREF='?src=\ref[src];amount=10'>+</A> <A HREF='?src=\ref[src];amount=100'>+</A><BR>\n<A HREF='?src=\ref[src];remove=1'>Remove plasmatank</A> - <A HREF='?src=\ref[src];close=1'>Close</A></TT>")
		user << browse(dat, "window=flamethrowinger;size=600x300")
		onclose(user, "flamethrowinger")
		return
	...
	Topic(href,href_list[])
		if (href_list["close"])
			usr.machine = null
			usr << browse(null, "window=flamethrowinger")
			return
		...
		if (href_list["light"])
			...
			if(lit)
				...
				damtype = "brute"
		if (href_list["amount"])
			src.throwing_amount = src.throwing_amount + text2num(href_list["amount"])
			src.throwing_amount = max(50,min(5000,src.throwing_amount))
		if (href_list["remove"])
			if(!src.ptank)	return
...
/obj/item/weapon/flamethrowinger/proc/flame_turf(turflist)
...
/obj/item/weapon/flamethrowinger/proc/ignite_turf(turf/target)
